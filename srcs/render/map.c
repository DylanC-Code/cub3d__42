...
/*   Updated: 2025/07/25 23:11:11 by dcastor          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (++x_y[1] < game->scene.map_height)
	{
		...
		while (++x_y[0] < game->scene.map_width)
		{
			...
			if (game->scene.map[x_y[1] * game->scene.map_width
				+ x_y[0]] == WALL)
				put_scare(&image_map, x_y_off, 30, 0x00FFFFFF);
			else
				put_scare(&image_map, x_y_off, 30, 0x00000000);
		}
	}
	mlx_put_image_to_window(game->mlx, game->window, image_map.img, 0,
